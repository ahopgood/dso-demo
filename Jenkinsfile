...
//           }
    ...
    stage('Package') {
      parallel {
        ...
        stage('OCI Image BnP') {
          steps {
            container('kaniko') {
              sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --customPlatform=linux/amd64 --insecure --skip-tls-verify --cache=true --destination=docker.io/altairbob/dso-demo'
            }
          }
        }
      }
            ...
            stage('Image Linting') {
                steps {
                    container('docker-tools') {
                        sh 'dockle docker.io/altairbob/dso-demo'
                    }
                }
            }
            stage('Image Scan') {
    ...
    stage('Deploy to Dev') {
        environment {
            ...
            ARGO_SERVER = 'argocd-server.svc.cluster.local:32100'
        }
        steps {
            container('docker-tools') {
